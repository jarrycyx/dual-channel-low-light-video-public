...
def LSTMCell(input, hidden, w_ih, w_hh, b_ih=None, b_hh=None, linear_func=None):
    ...
    if input.is_cuda and linear_func is F.linear:
        ...
        return state(igates, hgates, hidden[1]) if b_ih is None else cuda_fused_gru_cell(igates, hgates, hidden[1], b_ih, b_hh)
...
def GRUCell(input, hidden, w_ih, w_hh, b_ih=None, b_hh=None, linear_func=None):
    ...
    if input.is_cuda and linear_func is F.linear:
        ...
        cuda_fused_gru_cell = torch._C._VariableFunctions._thnn_fused_gru_cell
        return state(gi, gh, hidden) if b_ih is None else cuda_fused_gru_cell(gi, gh, hidden, b_ih, b_hh)
